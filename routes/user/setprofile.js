...
router.put('/',  upload.single('img'), authUtils.isLoggedin, async (req, res) => {

    //토큰에서 유저 idx 가져오기
    const loginUserIdx = jwt.verify(req.headers.token).idx;

    let findUserIdxQuery = `SELECT profile_img, nickname FROM user WHERE user_idx=${loginUserIdx}`;
    ...
    if (!userResult){
        ...
        if(newNickname.length<=5){
            let updateUserInfoResult = await db.queryParam_Arr(updateUserInfoQuery, [req.file.location , req.body.nickname, loginUserIdx]);
